...
function createProvider(
	...
	opts?: { multiChatEnabled?: boolean; claudeEnabled?: boolean; preferAgentHost?: boolean; nativeCli?: boolean; hideCopilotCli?: boolean },
...
function createProviderWithConfig(
	...
	configService.setUserConfiguration(ClaudeNativeCliSettingId, opts?.nativeCli ?? false);
...
suite('CopilotChatSessionsProvider', () => {
	...
	test('sessionTypes excludes Claude when nativeCli is true', () => {
		// The native-CLI switch also routes Claude through the agent host, so
		// this provider must yield exactly as it does for preferAgentHost.
		const provider = createProvider(disposables, model, { claudeEnabled: true, nativeCli: true });
		assert.strictEqual(provider.sessionTypes.length, 2);
		assert.ok(!provider.sessionTypes.some(t => t.id === ClaudeCodeSessionType.id));
	});

	test('onDidChangeSessionTypes fires when nativeCli setting changes', () => {
		...
		configService.setUserConfiguration(ClaudeNativeCliSettingId, true);
		configService.onDidChangeConfigurationEmitter.fire({
			source: ConfigurationTarget.USER,
			affectedKeys: new Set([ClaudeNativeCliSettingId]),
			change: { keys: [ClaudeNativeCliSettingId], overrides: [] },
			affectsConfiguration: (key: string) => key === ClaudeNativeCliSettingId,
		});
