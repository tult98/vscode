...
export class ClaudeNativeInputComposer extends Disposable {
	...
	/** Agent-host sessions already warmed, keyed by resource, so each warms at most once. */
	...
	setModel(model: IChatModel | undefined, session: IActiveSession | undefined): void {
		...
		this._updateSendButtonState();
		// Warm the agent-host session so its slash commands / skills are ready in
		// the `/` picker before the first follow-up is composed.
	...
	private _createEditor(container: HTMLElement, overflowWidgetsDomNode: HTMLElement): void {
		...
		this._register(this._editor.onDidFocusEditorWidget(() => this._prewarmActiveSession()));
	...
	/**
	 * Best-effort warm-up of the active agent-host session so the host reports
	 * its slash commands / skills to the `/` completion provider (the host only
	 * materializes them lazily on warm or first send). Runs at most once per
	 * session resource.
	 */
	private _prewarmActiveSession(): void {
		const resource = this._session.get()?.resource;
		if (!resource || !isAgentHostTarget(getChatSessionType(resource))) {
			return;
		}
		...
		this.chatSessionsService.prewarmChatSession(resource).catch(err => {
			// Best-effort: drop the marker so a later focus can retry.
			this._prewarmedSessions.delete(key);
			this.logService.trace('[ClaudeNativeInputComposer] prewarmChatSession failed', err);
		});
	}
