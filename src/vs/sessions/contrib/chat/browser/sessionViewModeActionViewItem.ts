 ...
 *
 * The two states map onto terminal mode: TUI => terminal mode on; GUI => terminal
 * mode off (the normal GUI chat).
 */
...
export class SessionViewModeToolbarContribution extends Disposable implements IWorkbenchContribution {
	...
	constructor(
		...
		// Refresh the toolbar item whenever terminal mode flips so the active
		// segment stays in sync even if the view item is recreated.
		const onDidChangeMode = Event.fromObservableLight(terminalModeService.terminalMode);
